...
module.exports = class OverwatchDiscord {
  ...
  _buildDiscordNotif(errors, source, appInfo, agent, log) {
    ...
    try {
      ...
      for (const error of errors.valueSeq()) {
        report = new Report(error, source, appInfo, 'Discord');
  ...
  *send(channel, errors, source, appInfo, agent, log) {
    ...
    return yield this.sendRequest(channel, body, appInfo, null, log);
  }

  *sendRequest(discordHook, body, appInfo, retryAfter = null, log, next) {
    let options = {
    ...
    if (body.content.length > 2000) {
      ...
      const content = `Errors on ${appInfo} too big to be displayed, attached as file below:`;
      options.formData = {
        ...body,
        content,
        file: {
