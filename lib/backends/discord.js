...
module.exports = class OverwatchDiscord {
  ...
  _buildDiscordNotif(error, source, appInfo, agent, log) {
    ...
    try {
      ...
      if (source === 'exception') {
        discordNotif.content = `**Whoa there ! An exception has occured on ${appInfo}** at `;
      } else {
        discordNotif.content = `**Barrier, activated ! A suspicious behavior has been detected on ${appInfo}** at `;
      }
      const date = time.get(0);
      discordNotif.content += `${date.toLocaleString()}\n`;
      discordNotif.content += `\`\`\`${error.get('err')}\`\`\``;
      if (error.get('_xcraftOverwatch')) {
        ...
        for (let i = callStackSize; i > 0; i--) {
          discordNotif.content += `\tat ${goblinInfo.get(
            `id.${i}`
          )}.${goblinInfo.get(`quest.${i}`)}\n`;
        }
        discordNotif.content += `\tat ${goblinInfo.get(
          'callerGoblin.0'
        )}.${goblinInfo.get('callerQuest.0')}\`\`\`\n`;
      } else {
        discordNotif.content += `**module**: ${error.get('mod.0')}\n`;
      }
      ...
      log.err(
        `Error while building discord notification for overwatch ! Exception : ${JSON.stringify(
          ex,
          null,
          2
