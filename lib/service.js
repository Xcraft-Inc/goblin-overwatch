...
Goblin.registerQuest(goblinName, 'push-errors', function* (
  ...
  let errorExist = state.get(`errors.${source}`);
  if (!errorExist) {
    // If error hash doesn't exist push in state
    errors[0].time = [errors[0].time];
    yield quest.me.change({
      path: `errors.${source}`,
      newValue: fromJS(errors[0]),
    });
  } else {
    // Push timestamp of all errors in the same error
...
Goblin.registerQuest(goblinName, 'send-errors', function* (quest) {
  ...
  for (const [key, error] of state.get('errors.exception', {})) {
