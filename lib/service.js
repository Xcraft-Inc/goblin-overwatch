...
Goblin.registerQuest(goblinName, 'send-errors', function* (quest, next) {
  const state = quest.goblin.getState();
  for (const errorId of state.get('errors.exception').keys()) {
  ...
  for (const errorId of state.get('errors.hazard').keys()) {
    for (const [backendKey, _] of backends) {
      quest.me.sendErrorsByBackend(
        {
          errorId,
...
Goblin.registerQuest(goblinName, 'send-errors-by-backend', function* (
  ...
  for (const channel of channels.get(backendKey, [])) {
    yield quest.me.sendError({
      backend,
      channel,
      error: state.get(`errors.${mode}.${errorId}`),
      mode,
      appInfo,
      agent,
      next,
...
Goblin.registerQuest(goblinName, 'send-error', function* (
  quest,
  backend,
  channel,
  error,
  ...
  const {log} = quest;
  retryAfter = yield backend.send(channel, error, mode, appInfo, agent, log);
  if (retryAfter) {
    yield quest.me.sendError({
      backend,
      channel,
